...
class MyersDiff[T](equalizer: Equalizer[T])
    extends weaver.diff.DiffAlgorithm[T] {
  ...
  private def buildRevision(
    ...
    while (path != null &&
      ...
      val delta: weaver.diff.Delta[T] =
        if (original.size == 0 && revised.size != 0) {
          new weaver.diff.InsertDelta[T](original, revised)
        } else if (original.size > 0 && revised.size == 0) {
          new weaver.diff.DeleteDelta[T](original, revised)
        } else {
          new weaver.diff.ChangeDelta[T](original, revised)
        }
